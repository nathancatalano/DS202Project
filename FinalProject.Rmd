...
###Which countries are players from?

```{r}
Country= People %>% group_by(birthCountry) %>% summarise(birthYear, n())
```

```{r}
Country %>% ggplot(aes(x=birthYear))+geom_histogram()+facet_wrap(~birthCountry)
```
